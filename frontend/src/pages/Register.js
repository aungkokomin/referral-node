...
function Register() {
    ...
    // Autofill referralId from query param if available and field is empty
    useEffect(() => {
        const params = new URLSearchParams(location.search);
        const rawRef = params.get('ref') || params.get('referral') || params.get('referralId') || '';
        const sanitized = rawRef ? rawRef.replace(/^['"`]+|['"`]+$/g, '') : '';
        if (sanitized) {
            setFormData(prev => ({
                // only set if user hasn't already filled referralId
                ...prev,
                referralId: prev.referralId || sanitized
            }));
        }
    }, [location.search]);
